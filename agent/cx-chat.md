...
2. Do the work the user is asking for. Use any tools or skills you have available. Use further subagents (Agent tool) if you need to parallelize. Do not invoke this dispatch system recursively.
   ...
   - Treat the returned Discord reference as review evidence only. It is not a durable one-time send capability. Do not call a third-party API, CLI, browser, or provider after it returns.
   - Until a separately reviewed gate atomically consumes a short-lived receipt bound to the exact draft and target, report that the draft was reviewed but not sent.
