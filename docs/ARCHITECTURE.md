...
Threadkeep supports two Discord orchestrators with separate transport and state paths. They must not share bot identities, channels, or model sessions. They do share one macOS account, so this design is not hard operating-system isolation.
...
| Human view | Live `threadkeep-chat` tmux | Optional read-only `threadkeep-codex` monitor |
...
| in threadkeep-chat tmux     |                     | guild, channel, owner, bot,|
...
`threadkeep-chat` contains one Claude Code session with the Discord plugin. Its identity comes from `cx-chat-listener/CLAUDE.md`, with PreCompact and UserPromptSubmit hooks as additional anchors.
...
`com.threadkeep.codex-discord-bridge` runs `PYTHONPATH=. python3 -m codex_discord_bridge.main` under launchd. One process owns:
...
The optional `threadkeep-codex` tmux session runs only `codex_discord_bridge.monitor`. It opens SQLite read-only and never talks to Discord or App Server. The LaunchAgent continues after the monitor is detached or closed.
...
Those retention and capacity rules govern Threadkeep's SQLite ledger. App Server persists non-ephemeral Codex thread and rollout state separately under the isolated `CODEX_HOME`. Threadkeep does not currently prune that provider-managed state or count it toward `max_database_bytes`, so it can outlive the corresponding SQLite mapping and grow independently.
...
Every durable job, managed Discord thread mapping, Codex session scope, and delivery manifest is bound to a policy fingerprint. The fingerprint covers immutable Discord identities, the resolved workspace, sandbox mode, model, OpenAI provider, Ultra effort, rendered thread policy, built-in Threadkeep base-instruction hash, reviewed Codex version, native binary hash, experimental schema hash, the domain-separated ChatGPT `account/read` binding, the sealed canonical Vault P0 source and snapshot hashes, the exact shared-skill closure, and optional trusted-instruction path and content hash. A queued job from another fingerprint is cancelled, a running job becomes uncertain, and old managed threads, Codex sessions, and prepared deliveries are not resumed under the new policy. A login-principal change therefore crosses the same durable policy boundary without writing the reported email to the ledger or logs.

### App Server lifecycle

OpenAI describes App Server as the interface used to power rich Codex clients, with authentication, conversation history, approvals, and streamed agent events. Threadkeep starts it as a child process over local stdio with `--strict-config`.
...
The optional trusted instruction file is loaded by Threadkeep and embedded into App Server base instructions. It must be a current-user-owned, single-link regular UTF-8 file, no larger than 256,000 bytes, and outside the writable workspace by lexical, canonical, and filesystem-identity comparison. The identity check covers macOS data-volume aliases that path canonicalization does not collapse. Threadkeep opens each path component without following symlinks, requires trusted root-owned or current-user-owned ancestry, and rechecks the file identity, link count, and ancestry after each read. Both modes disable project documents and require `instructionSources: []`. Danger-full-access mode automatically selects, hashes, and injects the canonical Vault `CLAUDE.md` through this external trusted-instruction path. Files from the working directory remain untrusted task content.

### Sandbox policy

Threadkeep sets permissions at both thread and turn creation:

- The compatibility setting `workspace-write` selects custom profile `threadkeep-workspace-only`. It extends `:workspace`, denies filesystem `:root`, keeps the minimal read set, denies both temporary-directory aliases, disables agent-tool network, and uses exactly one configured runtime workspace root with no added writable roots.
...
Threadkeep base instructions distinguish a request from permission to carry it out:
...
If the HTTP response is lost after Discord accepted the message, a retry inside Discord's documented short nonce window uses the same nonce. After that window, Threadkeep exhaustively scans the destination history and accepts only one exact message from the configured bot with the recorded nonce and content hash. An exact match is confirmed without another POST. No match, multiple matches, malformed pagination, or changed bot content quarantines the chunk as ambiguous and prevents an automatic repost. This chooses at-most-once public delivery when Discord cannot prove the outcome. If a crashed job has a complete delivery, it can move from `uncertain` to `completed` without rerunning Codex.
...
New Codex installations default runtime state to `~/Library/Application Support/Threadkeep/codex-discord/`, outside the normal `~/.threadkeep` repository clone. An explicit state override or reused path must remain below the canonical `~/Library/Application Support/Threadkeep` root. The installer and runtime loader independently canonicalize the path and reject a wrong owner, group or world write access, symlinks, and non-directory components from the user's canonical home through the state directory. This keeps ChatGPT auth and App Server rollouts from being redirected into another Git checkout or synced working tree. The state contains:
